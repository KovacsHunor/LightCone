...
class GPUProgram {
   ...
   public:
	...
	GPUProgram(const char* const vertexShaderSource, const char* const fragmentShaderSource,
			   const char* const GeometrymetryShaderSource = nullptr) {
		create(vertexShaderSource, fragmentShaderSource, GeometrymetryShaderSource);
	}
	...
	void create(const char* const vertexShaderSource, const char* const fragmentShaderSource,
		...
		// Program l�trehoz�sa a forr�s sztringb�l, ha van Geometrymetria �rnyal�
		GLuint GeometrymetryShader = 0;
		if (GeometrymetryShaderSource != nullptr) {
			GeometrymetryShader = glCreateShader(GL_GEOMETRY_SHADER);
			if (!GeometrymetryShader) {
				printf("Error in Geometrymetry shader creation\n");
				exit(1);
			}
			glShaderSource(GeometrymetryShader, 1, (const GLchar**)&GeometrymetryShaderSource, NULL);
			glCompileShader(GeometrymetryShader);
			if (!checkShader(GeometrymetryShader, "Geometrymetry shader error")) return;
		}
		...
		if (GeometrymetryShader > 0) glAttachShader(shaderProgramId, GeometrymetryShader);
...
class glApp {
   ...
   public:
	...
	virtual void onMouseScroll(float amount, int pX, int pY) {}
