...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
	...
	if (action == GLFW_PRESS)
		pApp->onMousePressed((button == GLFW_MOUSE_BUTTON_LEFT) ? MOUSE_LEFT : MOUSE_RIGHT, (int)pX, (int)pY);
	else
		pApp->onMouseReleased((button == GLFW_MOUSE_BUTTON_LEFT) ? MOUSE_LEFT : MOUSE_RIGHT, (int)pX,
							  (int)pY);
}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
	double pX, pY;
	glfwGetCursorPos(window, &pX, &pY);
	pApp->onMouseScroll(yoffset, pX, pY);
}
...
glApp::glApp(unsigned int _majorNumber, unsigned int _minorNumber, unsigned int _windowWidth,
			 unsigned int _windowHeight, const char* _windowCaption) {
